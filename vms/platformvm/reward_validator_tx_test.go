...
func TestUnsignedRewardValidatorTxExecuteOnCommit(t *testing.T) {
	...
	// Get old balances
	oldBalance, err := djtx.GetBalance(vm.internalState, stakeOwners)
	if err != nil {
		t.Fatal(err)
	}
	...
	onCommitBalance, err := djtx.GetBalance(vm.internalState, stakeOwners)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnsignedRewardValidatorTxExecuteOnAbort(t *testing.T) {
	...
	onAbortBalance, err := djtx.GetBalance(vm.internalState, stakeOwners)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRewardDelegatorTxExecuteOnCommit(t *testing.T) {
	...
	oldVdrBalance, err := djtx.GetBalance(vm.internalState, vdrDestSet)
	assert.NoError(err)
	oldDelBalance, err := djtx.GetBalance(vm.internalState, delDestSet)
	assert.NoError(err)
	...
	commitVdrBalance, err := djtx.GetBalance(vm.internalState, vdrDestSet)
	assert.NoError(err)
	...
	commitDelBalance, err := djtx.GetBalance(vm.internalState, delDestSet)
	assert.NoError(err)
...
func TestRewardDelegatorTxExecuteOnAbort(t *testing.T) {
	...
	newVdrBalance, err := djtx.GetBalance(vm.internalState, vdrDestSet)
	assert.NoError(err)
	vdrReward, err := math.Sub64(newVdrBalance, oldVdrBalance)
	assert.NoError(err)
	assert.Zero(vdrReward, "expected delegatee balance not to increase")

	newDelBalance, err := djtx.GetBalance(vm.internalState, delDestSet)
	assert.NoError(err)
