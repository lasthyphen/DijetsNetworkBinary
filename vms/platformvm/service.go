...
import (
	...
	"github.com/lasthyphen/DijetsNetworkBinary/utils/json"
	"github.com/lasthyphen/DijetsNetworkBinary/utils/math"
	"github.com/lasthyphen/DijetsNetworkBinary/utils/wrappers"
	"github.com/lasthyphen/DijetsNetworkBinary/vms/components/djtx"
	"github.com/lasthyphen/DijetsNetworkBinary/vms/components/keystore"
	"github.com/lasthyphen/DijetsNetworkBinary/vms/platformvm/reward"
	"github.com/lasthyphen/DijetsNetworkBinary/vms/platformvm/stakeable"
	"github.com/lasthyphen/DijetsNetworkBinary/vms/platformvm/status"
...
func (service *Service) ExportKey(r *http.Request, args *ExportKeyArgs, reply *ExportKeyReply) error {
	...
	address, err := djtx.ParseServiceAddress(service.vm, args.Address)
	if err != nil {
...
type GetBalanceResponse struct {
	// Balance, in nDJTX, of the address
	...
	UTXOIDs            []*djtx.UTXOID `json:"utxoIDs"`
}
...
func (service *Service) GetBalance(_ *http.Request, args *GetBalanceRequest, response *GetBalanceResponse) error {
	...
	// Parse to address
	addrs, err := djtx.ParseServiceAddresses(service.vm, args.Addresses)
	if err != nil {
		return err
	}

	utxos, err := djtx.GetAllUTXOs(service.vm.internalState, addrs)
	if err != nil {
...
func (service *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, response *api.GetUTXOsReply) error {
	...
	addrSet, err := djtx.ParseServiceAddresses(service.vm, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = djtx.ParseServiceAddress(service.vm, args.StartIndex.Address)
		if err != nil {
...
func (service *Service) GetStakingAssetID(_ *http.Request, args *GetStakingAssetIDArgs, response *GetStakingAssetIDResponse) error {
	...
	response.AssetID = service.vm.ctx.DJTXAssetID
	return nil
}
...
func (service *Service) AddValidator(_ *http.Request, args *AddValidatorArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := djtx.ParseServiceAddresses(service.vm, args.From)
	if err != nil {
		return err
	}

	// Parse the reward address
	rewardAddress, err := djtx.ParseServiceAddress(service.vm, args.RewardAddress)
	if err != nil {
	...
	if args.ChangeAddr != "" {
		changeAddr, err = djtx.ParseServiceAddress(service.vm, args.ChangeAddr)
		if err != nil {
...
func (service *Service) CreateSubnet(_ *http.Request, args *CreateSubnetArgs, response *api.JSONTxIDChangeAddr) error {
	...
	controlKeys, err := djtx.ParseServiceAddresses(service.vm, args.ControlKeys)
	if err != nil {
		return err
	}
...
// ExportDJTXArgs are the arguments to ExportDJTX
type ExportDJTXArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader

	// Amount of DJTX to send
	Amount json.Uint64 `json:"amount"`

	// Chain the funds are going to. Optional. Used if To address does not include the chainID.
	TargetChain string `json:"targetChain"`

	// ID of the address that will receive the DJTX. This address may include the
	// chainID, which is used to determine what the destination chain is.
	To string `json:"to"`
}

// ExportDJTX exports DJTX from the P-Chain to the X-Chain
// It must be imported on the X-Chain to complete the transfer
func (service *Service) ExportDJTX(_ *http.Request, args *ExportDJTXArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Debug("Platform: ExportDJTX called")
...
// ImportDJTXArgs are the arguments to ImportDJTX
type ImportDJTXArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader
...
// ImportDJTX issues a transaction to import DJTX from the X-chain. The DJTX
// must have already been exported from the X-Chain.
func (service *Service) ImportDJTX(_ *http.Request, args *ImportDJTXArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Debug("Platform: ImportDJTX called")
	...
	to, err := djtx.ParseServiceAddress(service.vm, args.To)
...
type GetStakeReply struct {
	...
	// Each is of type djtx.TransferableOutput
...
func (service *Service) getStakeHelper(tx *Tx, addrs ids.ShortSet) (uint64, []djtx.TransferableOutput, error) {
	var outs []*djtx.TransferableOutput
	switch staker := tx.UnsignedTx.(type) {
	case *UnsignedAddDelegatorTx:
		outs = staker.Stake
	case *UnsignedAddValidatorTx:
	...
	var (
		...
		err               error
		stakedOuts        = make([]djtx.TransferableOutput, 0, len(outs))
	)
	// Go through all of the staked outputs
	for _, stake := range outs {
		// This output isn't DJTX. Ignore.
		if stake.AssetID() != service.vm.ctx.DJTXAssetID {
			continue
		}
		out := stake.Out
...
// GetStake returns the amount of nDJTX that [args.Addresses] have cumulatively
// staked on the Primary Network.
//
// This method assumes that each stake output has only owner
// This method assumes only DJTX can be staked
...
func (service *Service) GetStake(_ *http.Request, args *GetStakeArgs, response *GetStakeReply) error {
	...
	var (
		totalStake uint64
		stakedOuts = make([]djtx.TransferableOutput, 0, len(stakers))
	)
...
// GetMinStake returns the minimum staking amount in nDJTX.
