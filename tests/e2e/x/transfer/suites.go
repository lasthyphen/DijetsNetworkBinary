...
import (
	...
	"github.com/lasthyphen/DijetsNetworkBinary/tests"
	"github.com/lasthyphen/DijetsNetworkBinary/tests/e2e"
	...
	"github.com/lasthyphen/DijetsNetworkBinary/wallet/subnet/primary"
	"github.com/lasthyphen/DijetsNetworkBinary/wallet/subnet/primary/common"
)

var keyFactory crypto.FactorySECP256K1R

var _ = e2e.DescribeXChain("[Virtuous Transfer Tx DJTX]", func() {
	ginkgo.It("can issue a virtuous transfer tx for DJTX asset", func() {
		...
		ginkgo.By("issue regular, virtuous X-Chain tx should succeed", func() {
			...
			djtxAssetID := baseWallet.X().DJTXAssetID()
			ewoqPrevBalX := balances[djtxAssetID]
			tests.Outf("{{green}}ewoq wallet balance:{{/}} %d\n", ewoqPrevBalX)

			balances, err = randWallet.X().Builder().GetFTBalance()
			gomega.Expect(err).Should(gomega.BeNil())

			randPrevBalX := balances[djtxAssetID]
			...
			balances, err = ewoqWallet.X().Builder().GetFTBalance()
			gomega.Expect(err).Should(gomega.BeNil())
			ewoqCurBalX := balances[djtxAssetID]
			tests.Outf("{{green}}ewoq wallet balance:{{/}} %d\n", ewoqCurBalX)

			balances, err = randWallet.X().Builder().GetFTBalance()
			gomega.Expect(err).Should(gomega.BeNil())
			randCurBalX := balances[djtxAssetID]
			tests.Outf("{{green}}ewoq wallet balance:{{/}} %d\n", randCurBalX)
