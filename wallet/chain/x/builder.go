...
func (b *builder) NewBaseTx(
	...
	djtx.SortTransferableOutputs(outputs, Parser.Codec()) // sort the outputs

	return &txs.BaseTx{BaseTx: djtx.BaseTx{
...
func (b *builder) NewCreateAssetTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.DJTXAssetID(): b.backend.CreateAssetTxFee(),
	}
	ops := common.NewOptions(options)
...
func (b *builder) NewImportTx(
	...
	var (
		...
		outputs      = make([]*djtx.TransferableOutput, 0, len(importedAmounts))
		importedDJTX = importedAmounts[djtxAssetID]
	)
	if importedDJTX > txFee {
	...
	djtx.SortTransferableOutputs(outputs, Parser.Codec())
	return &txs.ImportTx{
		BaseTx: txs.BaseTx{BaseTx: djtx.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: b.backend.BlockchainID(),
...
func (b *builder) spend(
	...
	djtx.SortTransferableOutputs(outputs, Parser.Codec()) // sort the change outputs
