...
func TestBaseTxSemanticVerifyUnauthorizedFx(t *testing.T) {
	...
	fx.InitializeF = func(vmIntf interface{}) error {
		vm := vmIntf.(secp256k1fx.VM)
		return vm.CodecRegistry().RegisterType(&djtx.TestTransferable{})
	}
...
func TestBaseTxSemanticVerifyPendingWrongAssetID(t *testing.T) {
	...
	tx := &txs.Tx{UnsignedTx: &txs.BaseTx{
		BaseTx: djtx.BaseTx{
			NetworkID:    networkID,
			BlockchainID: chainID,
			Ins: []*djtx.TransferableInput{{
...
func TestExportTxSemanticVerify(t *testing.T) {
	...
	genesisTx := GetDJTXTxFromGenesisTest(genesisBytes, t)
	djtxID := genesisTx.ID()
	rawTx := &txs.Tx{UnsignedTx: &txs.ExportTx{
	...
	err = rawTx.Visit(&txSemanticVerify{
		tx: utx.Tx,
		vm: vm,
	})
	if err != nil {
		...
		BaseTx: txs.BaseTx{BaseTx: djtx.BaseTx{
			...
			Ins: []*djtx.TransferableInput{
				...
				},
				{
					...
					Asset: djtx.Asset{ID: assetID}, // This asset doesn't exist
					In: &secp256k1fx.TransferInput{
						Amt:   startBalance,
						Input: secp256k1fx.Input{SigIndices: []uint32{0}},
					},
...
func TestExportTxSemanticVerifyInvalidFx(t *testing.T) {
	...
	utx.Tx.Creds[0].Verifiable = &djtx.TestVerifiable{}
	err = rawTx.Visit(&txSemanticVerify{
		tx: utx.Tx,
		vm: vm,
	})
