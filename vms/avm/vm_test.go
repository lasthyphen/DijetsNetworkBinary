...
import (
	...
	"github.com/lasthyphen/DijetsNetworkBinary/database/mockdb"
	...
	"github.com/lasthyphen/DijetsNetworkBinary/vms/avm/states"
...
func NewContext(tb testing.TB) *snow.Context {
	...
	tx := GetDJTXTxFromGenesisTest(genesisBytes, tb)

	ctx := snow.DefaultContextTest()
	ctx.NetworkID = networkID
	ctx.ChainID = chainID
	ctx.DJTXAssetID = tx.ID()
...
func GetDJTXTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *txs.Tx {
	return GetCreateTxFromGenesisTest(tb, genesisBytes, "DJTX")
}
...
func NewTx(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	return NewTxWithAsset(t, genesisBytes, vm, "DJTX")
}
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*txs.Tx) {
	...
	return issuer, vm, ctx, []*txs.Tx{djtxTx, firstTx, secondTx}
}
